...
2. Plot **all individual (unaggregated) grid-cell T–S pairs** within each box as small semi-transparent grey background points to show the full spread of the data.
3. Overlay the **spatially and temporally averaged T–S profile** (mean temperature and mean salinity at each standard depth level, averaged across all grid cells and months within the season) as a coloured path with depth encoded on a continuous colour scale.
...
ggplot() +
  geom_point(data = ts_all,
             ...
             colour = "grey60", alpha = 0.12, size = 0.4) +
