...
# 7--9. Tidyverse skills (tidy data → transform → group + graphics)
...
load(here::here("data", "SAMOS", "other_data", "SACTN_mangled.RData"))
...
kelp <- read_csv(here::here("data", "SAMOS", "other_data", "laminaria.csv"))
...
if (!exists("SACTN")) {
  load(here::here("data", "SAMOS", "other_data", "SACTNmonthly_v4.0.RData"))
...
Using `SACTN`, compute a **temperature anomaly** per site as the deviation of each monthly temperature from that site's long-term mean (`temp − mean(temp, na.rm = TRUE)` within each `site`). Then create a histogram of anomalies and facet by `src`. **(/15)**
...
```{r fig-taskD-q11, echo=TRUE, eval=TRUE}
#| fig-height: 12
#| fig-width: 9
#| out-width: "100%"
