...
This exercise requires that you analyse the data provided in `data/SAMOS/other_data/SACTN_SAWS.csv` within `<that which shall not be named>`.
...
laminaria <- read_csv(here::here("data", "SAMOS", "other_data", "laminaria.csv"))
# Run this if 'laminaria.csv` has columns separated by ';'
laminaria <- read_csv2(here::here("data", "SAMOS", "other_data", "laminaria.csv"))
```
...
The default way is to give a plain path string, for example `"data/SAMOS/other_data/laminaria.csv"`. `here::here()` just constructs that path for you. So:

* `here::here("data", "SAMOS", "other_data", "laminaria.csv")`
* is equivalent to `"data/SAMOS/other_data/laminaria.csv"`
...
laminaria <- read.csv(here::here("data", "SAMOS", "other_data", "laminaria.csv"))
```
...
write_csv(site_by_region, path = here::here("data", "SAMOS", "other_data", "kelp_summary.csv"))
```
...
```{r eval=FALSE}
read_csv(here::here("data", "SAMOS", "other_data", "laminaria.csv"))
```
