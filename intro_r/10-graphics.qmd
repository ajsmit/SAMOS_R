...
woa_1 <- woa |> 
  na.omit() |> 
  select(-source, -unit) |> 
  filter(variable %in% c("temperature", "salinity"), month == 2, depth_m == 0) |> 
  pivot_wider(names_from = variable, values_from = value)
  # group_by(lat, lon) |>
  # summarise(salinity = mean(salinity), temperature = mean(temperature)) |>
  # ungroup()
```
...
So what is that code doing? We may see from @fig-woa-ts-basic that it is creating a dot for every grid cell, placing **salinity** on the x‑axis and **temperature** on the y‑axis (a very common oceanography view). 
...
However, we *can* add an extra aesthetic mapping to include a third variable. In @fig-woa-ts-oxygen we colour points by **dissolved oxygen** (surface, February). This is an example of mapping a continuous variable to a continuous colour scale.
...
Do any patterns appear to emerge in @fig-woa-ts-oxygen? Typically oxygen is higher in cooler surface waters (and lower in warmer waters), but the relationship is not purely linear because circulation and biology both matter.

We can still add a simple best‑fit line through the points to demonstrate `geom_smooth()` (@fig-woa-ts-oxygen-lm):
...
Why did no legend appear in @fig-woa-constant-colour? Because the colour was set *outside* `aes()`, so ggplot treats it as a fixed setting, not a mapping. If you put a constant inside `aes()` (for example, `aes(colour = "steelblue")`), ggplot thinks you are mapping a category called “steelblue” and will create a legend.
...
Notice that in @fig-woa-size-mapped we mapped point size inside `aes(size = nitrate)`. That creates a size scale and legend. Any size you set *outside* `aes()` would be constant for all points.
...
Notice that in @fig-woa-labels-legend, when we place the legend at the bottom of the figure ggplot automatically makes it horizontal for us. Why do we use 'colour' inside of `labs()` to change the legend title?

# Reusable WOA examples (for other chapters)

The following small set of plots is deliberately designed to be copy‑pasted and adapted in later chapters: a spatial temperature map (@fig-woa-core-map-temp), a T–S scatter (@fig-woa-core-ts), and mean vertical profiles (@fig-woa-core-profile).
