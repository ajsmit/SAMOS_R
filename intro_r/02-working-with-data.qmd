...
`C:\Users\Student\Documents\SAMOS_R\data\SAMOS\processed\woa18_sa_core_1deg_monthly.csv`

macOS / Linux:

`/home/student/SAMOS_R/data/SAMOS/processed/woa18_sa_core_1deg_monthly.csv`
...
* `data/SAMOS/processed/woa18_sa_core_1deg_monthly.csv` refers to the processed monthly oceanographic dataset inside the `data/SAMOS/processed` sub-directory.
* `data/SAMOS/raw/woa18/temperature/woa18_decav_t01mn01.csv.gz` refers to a raw WOA18 temperature file inside `data/SAMOS/raw/woa18/temperature/`.
...
* `../raw/woa18/temperature/woa18_decav_t01mn01.csv.gz` — if written from inside `data/SAMOS/processed/`, this goes up one level to `data/SAMOS/` and then down into `raw/woa18/temperature/`.
* `../../processed/woa18_sa_core_1deg_monthly.csv` — if written from inside `data/SAMOS/raw/woa18/`, this goes up two levels to `data/SAMOS/` and then down into `processed/`.
...
* `data/SAMOS/raw/` ... (raw WOA18 source files, organised by variable: `temperature/`, `salinity/`, `oxygen/`, etc.)
* `data/SAMOS/processed/` ... (cleaned, integrated datasets ready for analysis)

If this directory is an RStudio Project, then all code can read data using short relative paths:
...
* `SAMOS_R/`
  ...
  * `data/SAMOS/raw/` ... (raw source files, organised by variable)
  * `data/SAMOS/processed/` ... (cleaned datasets ready for analysis)
...
::: callout-important
## Do This Now
1. Open the `SAMOS_R` project in RStudio. From the R Console, run `getwd()` and confirm that it returns the path to the `SAMOS_R` project root.

2. From the Console, use a relative path to read the processed dataset: `read.csv("data/SAMOS/processed/woa18_sa_core_1deg_monthly.csv")`. Then construct its absolute path on your machine. Which one would still work on another computer?

3. From the Console, try reading one of the raw files: `read.csv("data/SAMOS/raw/woa18/temperature/woa18_decav_t01mn01.csv.gz")`. Confirm the file loads successfully.

4. Deliberately break a path by misspelling a directory name (e.g., `data/SAMOS/procesed/woa18_sa_core_1deg_monthly.csv`). Observe the error message. Correct the spelling and rerun.

5. (Deliberately bad practice) Use `setwd("data/SAMOS/processed")` to point R directly at the processed data directory. Now try reading `woa18_sa_core_1deg_monthly.csv` without a path prefix, then try reading a raw file from `data/SAMOS/raw/`. Explain why this approach fails as a general strategy.
...
1. In RStudio, create a new project: **File → New Project → New Directory → New Project**.
   Name it `SAMOS_file_formats_exercise` and create it in a location you can find again (*e.g.*, Documents).
...
4. Decide on a file naming pattern you will follow (later; see below) for exports in this exercise. Use this pattern exactly:
   ...
   *Note:* `.scsv` here is just a human-visible label meaning “semicolon CSV”; it is still a plain text file, like a CSV file.
...
8. Save the Excel workbook into your project folder under `data/other/` with this name:
...
1. Export to **CSV (comma delimited)** using the naming scheme defined above:
   ...
   * Save into the project’s `data/SAMOS/` folder as:
     `survey_demo_YYYYMMDD_v01.csv`
...
* If it does *not* offer that option, do this instead:
  ...
  2. Save a new copy into the same `data/SAMOS/` folder named `survey_demo_YYYYMMDD_v01.scsv`.
...
12. In your file explorer (Windows Explorer / Finder), navigate to the project’s `data/SAMOS/` folder and confirm that these three files exist there:
...
file_csv  <- "data/SAMOS/survey_demo_YYYYMMDD_v01.csv"
file_tsv  <- "data/SAMOS/survey_demo_YYYYMMDD_v01.tsv"
file_scsv <- "data/SAMOS/survey_demo_YYYYMMDD_v01.scsv"
